---
title: "foundations in r pt 2 cali keller"
...
for(i in 1:nrow(ChickWeight)){
  if(ChickWeight$weight[i]>100){
    ChickWeight$mass[i]<- "yes"
  } else{ChickWeight$mass[i]<-"no" }
}
```
for(){
  if(){
...
```
```
based on the island the penguins are on, fill in a new column that tells us who's favorite it is 
Tejiri = Dream
Sawyer = Biscoe
Rachael = Torgersen 
```{r}
...
for(i in 1:nrow(penguins)){
  ...
  if(penguins$island[i]=="Torgersen"){
    penguins$studentFavorite[i]<- "Rachael"
  }
}
```
