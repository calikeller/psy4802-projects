...
for(i in 1:nrow(mtcars)){
  if(mtcars$mpg[i] > 25){
  mtcars$mpgAbove25[i] <- "good mileage"}
    else{
  mtcars$mpgAbove25[i] <- "bad mileage"
}
}
```
### else statement
...
mtcars$above25testagain <- ifelse(mtcars$mpg > 25, "good mileage", "bad mileage")
...
data(ChickWeight)
for(i in 1:nrow(ChickWeight)){
  if (ChickWeight$weight[i] > 100){ChickWeight$over100[i] <- "yes"}
  else{ChickWeight$over100[i] <- "no"}
}
```
for(){
  if(){
...
for(i in 1:nrow(mtcars)){
  ...
  else{mtcars$transmission[i] <- "automatic"}
}
```
