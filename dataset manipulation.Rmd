...
```{r}
library(readr)
aggresssiondata <- read_csv("PSY4802-AggressionSampleData.csv") #more reproducible
```
```{r}
getwd()
setwd(dir = "~/Downloads/")
setwd(dir = "~/Desktop/PSY4802- Fall 2025")
```
...
```{r}
aggressionLongDat<- pivot_longer(agressiondata, cols = c("baselineAggression", "postAggression"),names_to="timePoint", values_to="aggressionScore")
...
```{r}
longDat2<- gather(agressiondata, key= "timePoint",value = "aggressionScore", c("baselineAggression", "postAggression"))
...
```{r}
longdat3 <- melt(agressiondata, id.vars = c("subjNo", "ageGroup", "parentReportGender", "condition"), measure.vars = c("baselineAggression", "postAggression"), variable.name = "timepoint", value.name = "aggressionScore")
```

###practice: 
long format is when you combine repeated measurements

1. open the billboard dataset. 
```{r}
data(billboard)
#to find column positions
colnames(billboard)
...
```{r}
chickiewide <- pivot_wider(ChickWeight, id_cols = c("Chick", "Diet"), names_from = "Time", values_from = "weight")
#can only use a column name once
a <- pivot_wider(ChickWeight, id_cols = (1:2), )
```

3. Convert from wide to long. 
```{r}
chickLong <- pivot_longer(chickiewide, cols = c(colnames(chickiewide)[3:14]), names_to = "Time", values_to = "weight")
#contains ~20 more rows than original

chickLong2 <- na.omit(chickLong)
# is.na will help you remove specific nas from
```
...
```{r}
sleepywide <- pivot_wider(sleep, id_cols = "ID", names_from = "group", values_from = "extra")
```


4) Go back to the chick weight dataset and take all data from chick 3 and put it in wide format 
```{r}
chickie3 <- chickiewide[3,]
